...
export function Chatbot() {
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    // 1. Search local knowledge base for matches
    const matchedFaq = faqs.find((f) =>
      f.keywords.some((kw) => lowercaseQuery.includes(kw.toLowerCase()))
    );
    ...
    // 2. Call /api/chat endpoint (Gemini AI API)
    try {
    ...
    // 3. Fallback message with direct WhatsApp link if query not resolved
